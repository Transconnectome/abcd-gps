...
df_targets = pd.read_csv("/root/capsule/data/xgb_synthetic.csv")
demo = pd.read_csv("/root/capsule/data/demo_synthetic.csv")
gps_eur = pd.read_csv("/root/capsule/data/gps_eur_synthetic.csv")
subj_list = pd.read_csv('/root/capsule/data/subjectlist_EUR_100.csv')
...
discrete_y_vars = ['any_psych_dx_p_base', 'adhd_p_base', 'any_dep_dx_p_base',
                   'any_anx_dx_p_base', 'suicidal_behav_p_base', 'any_psych_dx_p_2yr',
                   'adhd_p_2yr', 'any_dep_dx_p_2yr', 'any_anx_dx_p_2yr',
                   'suicidal_behav_p_2yr', 'any_psych_dx_y_base', 'any_dep_dx_y_base',
                   'any_anx_dx_y_base', 'suicidal_behav_y_base', 'any_psych_dx_y_2yr',
                   'any_dep_dx_y_2yr', 'any_anx_dx_y_2yr', 'suicidal_behav_y_2yr']
...
data_eur = (
    df_targets
    .merge(demo_2, on="subjectkey", how="right")
    .merge(gps_eur, on="subjectkey", how="right")
)
...
def split_data_for_binary_classification(data, outcome_var, test_size=0.2):
    X = data.drop(columns=[outcome_var])
    y = data[outcome_var]

    if 'subjectkey' not in X.columns:
        raise ValueError("The dataset must include a 'subjectkey' column.")
...
X_train, X_test, y_train, y_test, train_subjectkeys, test_subjectkeys = split_data_for_binary_classification(data_eur_encoded, outcome_var)
...
base_dir = "/root/capsule/results/4_prediction/"
os.makedirs(base_dir, exist_ok=True)
plot_dir = "/root/capsule/results/4_prediction/plot"
...
pd.DataFrame(train_subjectkeys, columns=["subjectkey"]).to_csv(
    os.path.join(save_dir, "train_subjectkeys.csv"), index=False
)
pd.DataFrame(test_subjectkeys, columns=["subjectkey"]).to_csv(
    os.path.join(save_dir, "test_subjectkeys.csv"), index=False
)
...
y_train_scaled.to_csv(os.path.join(save_dir, "y_train_scaled.csv"), index=False)
y_test_scaled.to_csv(os.path.join(save_dir, "y_test_scaled.csv"), index=False)
