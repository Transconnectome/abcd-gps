...
# Load synthetic data
smri <- read_csv('smri_synthetic.csv')  # sMRI data (9,958 x 390)
gps <- read_csv('gps_eur_synthetic.csv') %>% 
  filter(set == 'test', ethnic_g == 'EUR')  # Filter test set and EUR ethnicity (4,254 x 37)
pheno <- read_csv('pheno_synthetic.csv')         # Phenotype data

# Load subject list
list_subj <- read_csv('subjectlist_EUR_100.csv') %>% rename(subjectkey = x)
...
saving_dir <- "/root/capsule/results/3_cca_2block"
